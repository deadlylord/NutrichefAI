    ...
    recipeSuggestions: {
      ...
      items: {
        ...
        properties: {
          ...
          youtubeQuery: { type: Type.STRING, description: 'Texto de búsqueda optimizado para YouTube para encontrar un video de esta receta (ej. "Receta bandeja paisa paso a paso").' }
        },
        required: ['title', 'description', 'ingredientsUsed', 'instructions', 'youtubeQuery']
      }
    }
  },
...
const mealSchema = {
    ...
    properties: {
        ...
        youtubeQuery: { type: Type.STRING, description: 'Query de búsqueda para YouTube (ej. "Cómo hacer Ajiaco Santafereño").' },
        foodBalance: {
            type: Type.OBJECT,
            properties: {
                vegetables: { type: Type.STRING, enum: ['high', 'medium', 'low'] },
                fruits: { type: Type.STRING, enum: ['high', 'medium', 'low'] },
                protein: { type: Type.STRING, enum: ['high', 'medium', 'low'] }
            },
            required: ['vegetables', 'fruits', 'protein'],
            description: 'Nivel de presencia de estos grupos en el plato.'
        }
    },
    required: ['name', 'calories', 'protein', 'carbs', 'fat', 'micronutrients', 'imagePrompt', 'instructions', 'youtubeQuery', 'foodBalance']
};
...
export const analyzeGroceryImages = async (
  ...
  const prompt = `
    ...
    2.  **recipeSuggestions**: 3 recetas usando estos ingredientes, incluyendo una query para buscar en youtube.
  `;
...
export const generateWeeklyMealPlan = async (
    ...
    const prompt = `
        ...
        5. Incluye 'foodBalance' (high/medium/low) para Verduras, Frutas, Proteína en cada plato.
        6. Incluye 'youtubeQuery' para buscar la receta.
        7. JSON estricto.
    `;
...
export const findMealImage = async (mealName: string): Promise<string | null> => {
    const prompt = `
        Busca una imagen real, pública y de alta calidad para el plato de comida: "${mealName}".
        Encuentra una URL directa que termine en .jpg, .png o .webp de un sitio como Wikimedia Commons, Food Network, o blogs de cocina populares que permitan visualización.
        
        Responde SOLAMENTE con la URL de la imagen en formato texto plano.
        Si no encuentras una URL directa de imagen, devuelve "null".
    `;
    ...
    try {
        ...
        console.error("Error searching for meal image:", error);
        return null;
    }
};
...
export const getFoodNutritionalInfo = async (foodName: string) => {
    const schema = {
        type: Type.OBJECT,
        properties: {
    ...
    const prompt = `
        Estima la información nutricional para 1 porción normal de: "${foodName}".
        Devuelve JSON.
    `;
...
export const searchFoodDatabase = async (query: string) => {
    ...
    const prompt = `
        Busca alimentos que coincidan con "${query}".
        Devuelve 5 opciones variadas con su información nutricional aproximada.
        Si la búsqueda es genérica (ej. "proteína"), sugiere alimentos altos en eso.
        Contexto: Comida colombiana y general.
        Responde en JSON.
    `;
